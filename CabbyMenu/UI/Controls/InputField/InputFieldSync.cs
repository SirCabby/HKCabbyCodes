...
namespace CabbyMenu.UI.Controls.InputField
{
    ...
    public static class InputFieldSync
    {
        ...
        public static BaseInputFieldSync<T> Create<T>(ISyncedReference<T> inputValue, KeyCodeMap.ValidChars validChars, Vector2 size, int characterLimit = Constants.DEFAULT_CHARACTER_LIMIT, T minValue = default(T), T maxValue = default(T))
        {
            Type type = typeof(T);
